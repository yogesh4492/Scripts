...
def check(
    ...
    if not skip_content:
        ...
        potential_dupes = find_potential_duplicates_by_size_etag(df)
        
        if not potential_dupes.empty:
            # Only hash files that have matching sizes
            typer.echo(f"\n⚡ Hashing only {len(potential_dupes)} potential duplicate files (skipping {len(df) - len(potential_dupes)} unique sizes)")
            
            # Use partial hash by default for speed
            md5_hashes = parallel_hash(bucket, potential_dupes.to_dict('records'), threads, use_partial=not full_hash)
