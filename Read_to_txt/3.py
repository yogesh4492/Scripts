    ...
    with open(file_path, "r") as f:
        for line in f:
            ...
            if not parts:
                continue
            path = parts[-1]
            normalized = path.replace("ROW_237", "projects/row_237")
            paths[normalized] = line.strip()
    return paths

def main():
    original = extract_paths("row237_original.txt")
    copy = extract_paths("row237_copy.txt")
    ...
    with open("missing_full_lines_row237.txt", "w") as out:
        for line in missing:
            out.write(line + "\n")
    ...
    print("📝 Full details saved to 'missing_full_lines_row237.txt'")
