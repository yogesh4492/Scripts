...
def check_duplicates(df: pd.DataFrame):
    ...
    name_dupes = df[df.duplicated('Key', keep=False)]
    content_dupes = df[df.duplicated('MD5', keep=False)]
    return name_dupes, content_dupes


def save_to_excel(name_dupes, content_dupes, output_file: str):
    """Save duplicate results to Excel with two sheets."""
    with pd.ExcelWriter(output_file.replace('.csv', '.xlsx')) as writer:
        name_dupes.to_excel(writer, sheet_name='Duplicate_Names', index=False)
        content_dupes.to_excel(writer, sheet_name='Duplicate_Content', index=False)
    typer.echo(f"\n✅ Duplicate report saved: {output_file.replace('.csv', '.xlsx')}")
...
def check(
    ...
    output: str = typer.Option("s3_duplicates.csv", "--output", "-o", help="Output CSV/Excel file name"),
    ...
    if name_dupes.empty and content_dupes.empty:
        ...
        save_to_excel(name_dupes, content_dupes, output)
